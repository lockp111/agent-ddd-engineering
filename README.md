...
| 技能目录                                                            | 描述说明                                                                                                                                 |
| ------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **🚀 `full-ddd/`**<br>(全流程 DDD 编排)                              | 串联完整的 5 阶段 DDD 研发管线，强制执行节点审查。适合项目冷启动解析 PRD，一步步向最终代码演进。                                         |
| **📥 `importing-technical-solution/`**<br>(导入已有技术方案)         | 针对存在历史技术文档、系统设计或规范（ADR）的场景，反向抽取校验 DDD 合规性，完成模型对齐。                                               |
| **🎯 `extracting-domain-events/`**<br>(Phase 1: 领域事件抽取)        | 强大的 EventStorming 引导技能，强制 AI 从冗长需求中提取真实的 `Command` -> `Event` -> `Actor` 规则，拒绝凭空写代码。                     |
| **🌐 `mapping-bounded-contexts/`**<br>(Phase 2: 限界上下文映射)      | 引导 AI 按业务划分物理微服务与模块的准确边界，建立统一语言与防污染层（ACL），消灭共享“上帝类”。                                          |
| **📜 `designing-contracts-first/`**<br>(Phase 3: 契约优先设计)       | 强制约束跨界通信规则，要求 AI 在实现逻辑前必须先提交并冻结纯粹的接口契约（API/事件），阻断不良耦合。                                     |
| **🏗️ `architecting-technical-solution/`**<br>(Phase 4: 架构技术推演) | 阻断从设计直接跳进编写底层代码。强迫 AI 基于模型分类（核心域/支撑域）明确深度，针对存储、协议、一致性等 7 项维度做出决策并接受人类质询。 |
| **📦 `coding-isolated-domains/`**<br>(Phase 5: 领域层隔离编码)       | 强制实现六边形架构/洋葱架构，命令 AI 产出带有充分业务行为逻辑的“充血模型”，严惩注入 ORM 或 HTTP 等外部组件相关的脏代码。                 |
