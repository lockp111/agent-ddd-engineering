...
| -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **📦 `coding-isolated-domains/`**<br>(Isolated Domain Coding)   | Enforces Hexagonal/Onion architecture, requiring AI to generate "Rich Models" carrying sufficient business behavior, strictly prohibiting ORM/HTTP side effects.       |
| **📜 `designing-contracts-first/`**<br>(Contract First Design)  | Requires AI to define pure interface specifications (API / Domain Events) before implementation, followed by dependency inversion implementation by external adapters. |
| **🎯 `extracting-domain-events/`**<br>(Domain Event Extraction) | Powerful text analysis Prompt skills, guiding AI to extract `Command` -> `Event` -> `Actor` structures from lengthy requirement documents.                             |
| **🌐 `mapping-bounded-contexts/`**<br>(Bounded Context Mapping) | Guides AI in dividing physical boundaries of microservices/modules, establishing Anti-Corruption Layers (ACL) to prevent cross-module direct object referencing.       |
